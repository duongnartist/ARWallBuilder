...
class Wand {
    ...
    class func coneMaterials() -> [SCNMaterial] {
        let mat = SCNMaterial()
        mat.diffuse.contents = UIImage(named: "art.scnassets/s/wand_stock_diffuse.jpg")
...
//            mat.transparent.contents = UIImage(named: "art.scnassets/s/BeachBallTransparent.png")
    ...
    class func wandNode() -> SCNNode {
        ...
        let cone = SCNCone(topRadius: HANDLE_TOP_RADIUS, bottomRadius: HANDLE_BOTTOM_RADIUS, height: HANDLE_LENGTH)
    ...
    class func node() -> SCNNode {
        ...
        node.physicsBody = SCNPhysicsBody(type: SCNPhysicsBodyType.kinematic, shape: SCNPhysicsShape(geometry: physicsGeometry, options: nil))
